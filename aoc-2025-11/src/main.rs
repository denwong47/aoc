...
fn count_number_of_solutions(
    ...
    let solution_count =
        simple_graph::dfs_count::<models::DeviceId, models::Distance, models::Device>(
            private_devices
                .get(&start_id)
...
fn part_2_solutions_count(devices: &models::DeviceMap) -> anyhow::Result<usize> {
    ...
    let svr_to_dac_count = count_number_of_solutions(&devices, server_rack_id, dac_id, &[])
    ...
    let svr_to_out_through_fft_count = svr_to_dac_count
...
fn main() {
    ...
    let start = Instant::now();
    '_part2: {
        let solution_count = part_2_solutions_count(&devices)
